#version 330 core

in vec2 frag_uv;
flat in ivec4 frag_src_bounds;
flat in uint frag_cset;
flat in vec4 color_filter;

uniform usampler2DRect spritesheet;
uniform sampler2DRect palette;

out vec4 frag_color;

void main() {
    vec2 size = vec2(frag_src_bounds.zw - frag_src_bounds.xy);
    uint color_index = texelFetch(spritesheet, ivec2(size * frag_uv) + frag_src_bounds.xy).r % uint(textureSize(palette, 0).x);
    if (color_index == 0u) discard;
    frag_color = vec4(texelFetch(palette, ivec2(color_index, frag_cset)).rgb, 1.0) * color_filter;
}
